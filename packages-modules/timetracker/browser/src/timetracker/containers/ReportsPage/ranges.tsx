...
export interface IinputRnges {
  daysUpToToday?: any;
  daysStartingToday?: any;
  weekStartsOn?: any;
  firstDayOfWeek?: any;
}
export const inputRanges = (
  ...
  weekStartsOn) => [
  {
    ...
    getCurrentValue(range) {
      if (!isSameDay(range.endDate, defineds(weekStartsOn).endOfToday)) {
        return '-';
      }
      if (!range.startDate) {
        return '∞';
  ...
  },
  {
    ...
    getCurrentValue(range) {
      if (!isSameDay(range.startDate, defineds(weekStartsOn).startOfToday)) {
        return '-';
      }
      if (!range.endDate) {
        return '∞';
