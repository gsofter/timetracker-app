...
- `yarn watch-packages -- --scope @admin-layout/counter-module-*` - By adding package module you like to watch along with the dependent packages. If you have more packages to watch keep adding with `-- --scope packageA* --scope packageB`
...
```
lerna exec --scope=@admin-layout/counter-module-browser yarn watch
lerna exec --scope=@admin-layout/counter-module-server yarn watch
```
